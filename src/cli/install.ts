...
${formatPromptRules("### Retrieval routing", [
  ...
  `For a workspace-grounded question that explicitly asks about multiple stages, causes, or handoffs, keep the original wording in the primary hybrid \`query\` and add at most one supplemental \`vector\` or \`fts\` group for a distinct facet in the same \`${searchTool}\` call. Use \`vector\` for conceptual or state-flow facets; use \`fts\` only for exact names or terms already present in the request or retrieved evidence. Keep groups separate by omitting \`fuse\`, and use a small per-group \`limit\` (for example, 5). Do not invent repository-specific identifiers. For atomic questions or uncertain decompositions, use the normal single hybrid query.`,
